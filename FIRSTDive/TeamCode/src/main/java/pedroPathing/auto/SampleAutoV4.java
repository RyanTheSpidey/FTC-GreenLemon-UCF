...
@Disabled
@Autonomous
public class SampleAutoV4 extends OpMode {
    ...
    private static final Pose preloadPose = new Pose(125, /*10.2*/9.9, Math.toRadians(172));
    private static final Pose basketPose = new Pose(126, 10, Math.toRadians(/*175*/172));
    private static final Pose leftPose = new Pose(118, /*20*/20.4, Math.toRadians(220));
    private static final Pose rightPose = new Pose(122.5, /*14*/15, Math.toRadians(160));
    private static final Pose subPose = new Pose(78, 54, Math.toRadians(90));
    private static final Point subControlPoint = new Point(78, 28);
    private static final Pose returnPose = new Pose(124, 12, Math.toRadians(160));
    private static final Point returnControlPoint = new Point(78, 28);


    private Path scorePreload, park;
    private PathChain grabLeft, scoreLeft, grabRight, scoreRight, sub1, return1;
    ...
    public void buildPaths(){
        ...
        grabLeft = follower.pathBuilder()
                .addPath(new BezierLine(new Point(preloadPose), new Point(leftPose)))
                .setLinearHeadingInterpolation(preloadPose.getHeading(), leftPose.getHeading())
                .build();
    ...
    public void autonomousPathUpdate(){
        switch (pathState){
            ...
            case 1:
                ...
                if (actionTimer.getElapsedTimeSeconds() > 0.8 + outtakeStamp && outtakeState == -1){
                    outtakeState = 1;
                    outtakeStamp = actionTimer.getElapsedTimeSeconds();
                ...
                if (actionTimer.getElapsedTimeSeconds() > 1.2 + intakeStamp && intakeState == 1 && extendo.getCurrentPosition() > extendoMax - 20){
                    intakeState = 2;
                    intakeStamp = actionTimer.getElapsedTimeSeconds();
            ...
            case 6:
                ...
                if (intakeState == -101){
                    ...
                    if (!getDetectedSample().equals("yellow")) {
                        ...
                        setPathState(-1);
                        follower.setMaxPower(1);
                    } else if (extendo.getCurrentPosition() >= extendoMin){
                        extendo.setPower(0.05);
                        intakeState = 1;
                        intakeStamp = actionTimer.getElapsedTimeSeconds();
                        follower.setMaxPower(1);
                    }
