...
@Disabled
@Autonomous
public class SampleAutoV6 extends OpMode {
    ...
    private boolean failedFirst = false;
    ...
    private static final Pose middlePose = new Pose(124.5, 10.1, Math.toRadians(174));
    private static final Pose basketPose = new Pose(126, 10, Math.toRadians(/*175*/168));
    private static final Pose leftPose = new Pose(117.5, /*20*/20.6, Math.toRadians(220));
    private static final Pose rightPose = new Pose(122.5, /*14*/14.7, Math.toRadians(160));
...
//                        follower.setMaxPower(1);
                    ...
                    if (!captured && failedFirst) {
                        turnTo(105);
                        while (Math.abs(105 - Math.toDegrees(follower.getPose().getHeading())) > 3){

                        }
                        follower.breakFollowing();
                        extendo.setPower(-0.5);
                        while (!getDetectedSample().equals("yellow") && opmodeTimer.getElapsedTimeSeconds() < 28 && extendo.getCurrentPosition() > 150){
                            extendo.setTargetPosition(extendoRetracted);
                            extendo.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                            extendo.setPower(0.5); //0.2
                        }
                        if (extendo.getCurrentPosition() < 250 || opmodeTimer.getElapsedTimeSeconds() >= 28) {
                            setExtendo(extendoRetracted);
                            intakeElbow.setPosition(intakeElbowNeutral);
                            intakeFlexor.setPosition(intakeFlexorNeutral);
                            follower.followPath(return1, true);
