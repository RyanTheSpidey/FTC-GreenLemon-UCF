...
@Disabled
@Autonomous
public class SampleAutoV1 extends OpMode {
    ...
    public void buildPaths(){
        ...
        scoreSampleLeft = follower.pathBuilder()
                ...
                .setLinearHeadingInterpolation(sampleSpikeLeftPose.getHeading(), sampleBasketPose.getHeading())
                .build();
        ...
        switch (pathState){
            ...
            case 1:
                ...
                if (follower.getVelocityMagnitude() < 1 && follower.getCurrentTValue() > 0.7) {
                    ...
                    if (actionTimer.getElapsedTimeSeconds() > 0.3){ //0.5
            ...
            case 2:
                ...
                if (follower.getVelocityMagnitude() < 1 && follower.getCurrentTValue() > 0.7) {
                    ...
                    if (actionTimer.getElapsedTimeSeconds() > 0.3 && actionTimer.getElapsedTimeSeconds() < 0.6){
                        intakeElbow.setPosition(intakeElbowActive);
                    }
                    if (actionTimer.getElapsedTimeSeconds() > 0.6 && actionTimer.getElapsedTimeSeconds() < 0.9){
                        intakeGrabber.setPosition(intakeGrabberClosed);
                    }
                    if (actionTimer.getElapsedTimeSeconds() > 0.9 && actionTimer.getElapsedTimeSeconds() < 1.7) {
                        setExtendo(extendoTransfer);
                        intakeElbow.setPosition(intakeElbowTransfer);
                        intakeFlexor.setPosition(intakeFlexorTransfer);
                    }
                    if (actionTimer.getElapsedTimeSeconds() > 1.7 && actionTimer.getElapsedTimeSeconds() < 2.2){
                        setExtendo(extendoTransfer - extendoTransferAmount);
                    }
                    if (actionTimer.getElapsedTimeSeconds() > 2.2 && actionTimer.getElapsedTimeSeconds() < 2.4) {
                        setOuttakeGrabber(false);
                    }
                    if (actionTimer.getElapsedTimeSeconds() > 2.4 && actionTimer.getElapsedTimeSeconds() < 2.5) {
                        intakeGrabber.setPosition(intakeGrabberOpen);
                        setExtendo(extendoTransfer);
                    }
                    if (actionTimer.getElapsedTimeSeconds() > 2.5 && extendo.getCurrentPosition() > extendoTransfer - 50){
                        actionTimerActive = false;
                        setElevator(elevatorBasket);
